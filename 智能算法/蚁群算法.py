...
class ACO(object):
    def __init__(self, ant_num=50, maxIter=300, alpha=1, beta=5, rho=0.1, Q=1):
        ...
        ###########################
        self.path_seed = np.zeros(self.ants_num).astype(
            int)      # 记录一次迭代过程中每个蚂蚁的初始城市下标
        self.ants_info = np.zeros(
            (self.maxIter, self.ants_num))  # 记录每次迭代后所有蚂蚁的路径长度信息
        # 记录每次迭代后整个蚁群的“历史”最短路径长度
    ...
    def deal_data(self, filename):
        ...
        self.cities = list(City(float(item[0]), float(
            item[1])) for item in temp_list)     # 2. 构建城市列表
        self.city_dist_mat = np.zeros(
            (self.cities_num, self.cities_num))                  # 3. 构建城市距离矩阵
        for i in range(self.cities_num):
            A = self.cities[i]
            for j in range(i, self.cities_num):
                B = self.cities[j]
                self.city_dist_mat[i][j] = self.city_dist_mat[j][i] = Ant.calc_len(
                    A, B)
        # 4. 初始化信息素矩阵
        self.phero_mat = np.ones((self.cities_num, self.cities_num))
        # self.phero_upper_bound = self.phero_mat.max() * 1.2                              ###信息素浓度上限
        # 5. 初始化启发函数矩阵
        self.eta_mat = 1/(self.city_dist_mat +
                          np.diag([np.inf]*self.cities_num))
    ...
    def solve(self):
        ...
        while iterNum < self.maxIter:
            ...
            delta_phero_mat = np.zeros(
                (self.cities_num, self.cities_num))     # 初始化每次迭代后信息素矩阵的增量
            ##########################################################################
                ...
                for j in range(self.cities_num-1):                             # 对余下的城市进行访问
                    ...
                    for k in range(self.cities_num-len(tabu)):
                        up_proba[k] = np.power(self.phero_mat[city_index1][non_tabu[k]], self.alpha) * \
                            np.power(self.eta_mat[city_index1]
                                     [non_tabu[k]], self.beta)
                    # 每条可能子路径上的状态迁移概率
    ...
    def random_seed(self):
        if self.ants_num <= self.cities_num:                                   # 蚂蚁数 <= 城市数
            self.path_seed[:] = np.random.permutation(
                range(self.cities_num))[:self.ants_num]
        else:                                                                  # 蚂蚁数 > 城市数
            ...
            while temp_index + self.cities_num <= self.ants_num:
                self.path_seed[temp_index:temp_index +
                               self.cities_num] = np.random.permutation(range(self.cities_num))
                temp_index += self.cities_num
            temp_left = self.ants_num % self.cities_num
            if temp_left != 0:
                self.path_seed[temp_index:] = np.random.permutation(
                    range(self.cities_num))[:temp_left]
    ...
    def display(self):                                                         # 数据可视化展示
        ...
        plt.legend()
        plt.subplot(212)
        plt.plot(list(city.x for city in self.best_cities),
                 list(city.y for city in self.best_cities), 'g-')
        plt.plot(list(city.x for city in self.best_cities),
                 list(city.y for city in self.best_cities), 'r.')
        plt.xlabel('x')
        plt.ylabel('y')
